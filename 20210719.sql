...
-- MySQL과 마찬가지로 Oracle에서도 거의유사한 SELECT구문을 사용합니다.
...
-- Oracle은 문자열 자료형의 경우 대소문자구분을 합니다.

-- 급여가 5000이상이거나 혹은 jo_id거 'IT_PROG' 직원들을 조회해주세요.
SELECT employee_id, first_name, last_name
    FROM employees
...
-- 컬럼명이 원래는 기존에 테이블을 만들때 설정한 이름을 조회되지만
...
-- 컬럼명은 전체컬럼에 집어넣는경우 MYSQL과 마찬가지로 생략 가능합니다.
...
-- 바로 아래에 SELECT 구문은 NUMBER데이터만 결과로 나옵니다.
SELECT employee_id FROM employees;

-- 아래 구문은
-- test2 테이블 내부에, 88번라인 결과로 나온 숫자 전체를 집어넣습니다.
INSERT INTO test2 (SELECT employee_id FROM employees);

-- 92번라인은 실제로는 아래쪽 MYSQL의 구문처럼 처리됩니다.
...
-- 4개 컬럼을 전체 복사받는 test3 테이블을 생성한 후
-- 복사를 진행해주세요.
CREATE TABLE test3(
employee_id NUMBER,
...
-- UPDATE문
-- update set 을 이용해서 처리하면 mysql과 거의 유사한 문법을 사용합니다.
-- UPDATE SET은 그냥 처리하면 컬럼 전체값이 바뀝니다.
-- 따라서 WHERE와 함께 사용하는것이 일반적입니다.
SELECT * FROM test1;
...
-- WHERE 절이 들어가지 않는다면 전체 데이터가 삭제되기 때문에
...
-- MYSQL에는 없는 기능입니다.
-- rownum, rowid처럼 사용자가 직접 입력하지 않고 조회하는 컬럼을 가리켜서
-- 의사컬럼이라고 부릅니다.
...
SELECT * FROM employees;
-- EMPLOYEES에서는 JOB_ID(직무 구분), DEPARTMENT_ID(부서 구분) 등 여러
-- 집단을 대표하는 자료가 있습니다.
...
-- 각 부서별 평균 연봉을 구하되, 6000이상 ~ 9000미만으로 구해주세요.
...
-- 그러나 반드시 FOREIGN KEY 관계여야만 조인을 수행할 수 있는것은 아닙니다.
...
-- RIGHT OUTER JOIN - 오른쪽 테이블자료를 모두 살리기 때문에
...
-- 공통자료 10,20 뿐만아니라 왼쪽에 40, 오른쪽에 30이 모두 출력
